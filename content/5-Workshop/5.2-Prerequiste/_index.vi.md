...
### 5. Trình soạn thảo mã (Khuyến nghị: VS Code)
```bash
# macOS
...
```env
# Database (Aurora - điền sau khi thiết lập xong trên AWS)
DB_NAME=newsrag
DB_USER=postgres
...
> **Bảo mật:** Không bao giờ commit `.env` lên git. File này đã có trong `.gitignore`.
...
1. Cập nhật `AWS_REGION` trong `.env`
2. Đảm bảo model Bedrock `amazon.titan-embed-text-v2:0` có sẵn trong region của bạn (kiểm tra [Bedrock regions](https://docs.aws.amazon.com/bedrock/latest/userguide/models-regions.html))
...
**Tiếp theo:** [Thiết lập Hạ tầng trên AWS Console](5.3-Infrastructure/)